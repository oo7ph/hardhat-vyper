...
async function compile(vyperConfig, paths, artifacts) {
    ...
    for (const file of files) {
        ...
        if (await isAlreadyCompiled(file, paths, vyperVersion)) {
            console.log(`${emoji_1.emoji("✓ ")}${pathFromCWD} is already compiled`);
            continue;
        }
        console.log(`${emoji_1.emoji("⌛ ")}Compiling ${pathFromCWD}`);
...
async function isAlreadyCompiled(sourceFile, paths, vyperVersion) {
    ...
    const sourceName = path_1.default.relative(paths.root, sourceFile);
    const artifactPath = path_1.default.join(paths.artifacts, sourceName, `${contractName}.json`);
    if (!(await fs_extra_1.default.pathExists(artifactPath))) {
        return false;
    }
    const lastSourcesCtime = (await fs_extra_1.default.stat(sourceFile)).ctimeMs;
